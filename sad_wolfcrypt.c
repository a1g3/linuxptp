                              ...
                              const unsigned char *key, size_t key_len)
{
    ...
    switch (algorithm) {
        ...
        case ED25519:
            ...
            mac_data->ed25519_key = calloc(1, sizeof(ed25519_key));
            if (!mac_data->ed25519_key) {
             ...
             unsigned char *mac, size_t mac_len)
{
    ...
    if (mac_data->is_cmac) {
        ...
        if (wc_CmacFinal(mac_data->cmac, mac, (unsigned int *)&mac_len) != 0) {
        ...
        return mac_len;
    } else if (mac_data->is_ed25519) {
        if (mac_len > ED25519_SIG_SIZE) {
            pr_err("mac_len exceeds ED25519 signature size");
            return 0;
        }

        if (wc_ed25519_sign_msg(data, data_len, mac, (unsigned int *)&mac_len, mac_data->ed25519_key) != 0) {
               ...
               unsigned char *mac, size_t mac_len)
{
    ...
    if (!mac_data->is_ed25519) {
        ...
        if (wc_ed25519_verify_msg(mac, mac_len, data, data_len, &ret, mac_data->ed25519_key) != 0) {
